...
#Estruturas ============================================================
#Node: .space 28 #Este � o espa�o da estrutura n�
memory: .space 20000
TreeRoot: .space 4      #Este � o espa�o onde ser� guardado O endereco de memoria da raiz
valAlocation: .space 4  #alocndo espa�o para deslocamento de alocacao
valDecremento: .space 4 #O espa�o onde est� gravado o valor do decremento 
...
main:
	...
	li $a0, 0    #Zeraando $a0
	li $v0, 42
	li $a1, 501 #Gerando numeros aleat�rios entre 0 e 500
	syscall
...
inserindoValores:	
	...
	addi $a0, $t8, 0
	li $v0, 42
	li $a1, 1001 #Gerando numeros aleat�rios entre 0 e 1000	li $a1, 10000 #Gerando numeros aleat�rios entre 0 e 1000	syscall 
...
buscar: addi $sp, $sp, -4
	...
	beq $t0, 0, endLoopBusca #Se o n� for nulo
	...
	endLoopBusca:
		#final da fun��o buscar
		lw $ra, 0($sp)
		addi $sp, $sp, 4
		jr $ra
		
#$a1 O endereco de mem�ria da raiz
...
inserir:
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	#Com o endereco da raiz e o valor que eu quero inserir eu fa�o uma chamada de busca
	...
	beq $t1, 0, gravarRaiz #Se for nulo, passa para gravar na raiz
		...
		slt $t6, $s0, $t7#se o valor que eu  quero inserir � menor
		bne $t6, 1, elseMenor
		jal malloc
	...
	gravarRaiz:
		...
		jal malloc
		sw $v0, 0($a1) #salvando o endereco de mem�ria no espaco da memoria guardado para o node raiz
		lw $t6, 0($a1) #carregando o endereco do node raiz da mem�ria
...
#$a1 -> NoArvore
#$t0 -> ptr , 
#$t1 -> fEsq, $t2 ->registrador auxiliar para verifica��o
RSD: #Rotacao simples direita
	...
	lw $t2, 28($t1)  #carregando em registrdor auxiliar o endere�o Fdir 
	...
	beq $t2, 0, gravarNovaRaizRSD
		#Ser� utilixado um registradores $t3 e $t4
...
#$a1 -> NoArvore
#$t0 -> ptr , 
#$t1 -> fDir, $t2 ->registrador auxiliar para verifica��o	
RSE: #Rotacao simples esquerda
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	addi $t0, $a0, 0 #salvando o endereco em registrador
	lw $t1,28($t0)   #carregando endereco de filho Direita $t0
	lw $t2, 24($t1)  #carregando em registrdor auxiliar o endere�o Fdir (Filo Esq do filho Dir)
...
verificandoRotacoes:
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	beq $a0, 0, endVR # se o no n�o for nulo
	...
	bne $t0, 2, verDoisNeg
		...
		beq $t7, 1, elseVRE
			...
			seq $t7, $t0, -1
			bne $t7,1,elseVRE#Verificar Rota��o esquerda
	...
	bne $t0, -2, endVR
		...
		beq $t7, 1, elseVRD
			...
			seq $t7, $t0, 1
			bne $t7,1,elseVRD#Verificar Rota��o esquerda
...
#Fun��es para calcular fatores de balanceamento===================================
...
calcularFatorBalanceamento:
	...
	beq $t1, 0, elseEsqNull
		lw $t2, 4($t1) #Pegando altura do filho � esqueda
...
endCalcularFatorBal:
	...
	sw $t4, 16($t0) #salvando o FatBal no n�
	#verificando maior
	sge $t7, $t3, $t2
	bne $t7, 1, max 
...
#$a0 -> O node inserido
#$a1 -> o endereco de memoria da �rvore
#$t0 -> ptr; $t1, $t2 Registradores auxiliares
atualizarFatorBalanceamento:
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	addi $t0, $a0, 0#carregando o endereco de mem�ria do node raiz
#O seguinte loop atualiza o fator de balanceamento do nodes e performa rota��es
...
malloc:
	...
	#O endereco dinamicamente alocado est� em $v0
	lw $ra, 0($sp)		
	addi $sp, $sp, 4
	jr $ra	#retorna com o endereco de aloca��o em $v0

#a1 ->endereco do node raiz
#$t0 -> registrador auxiliar para guardar endereco de mem�ria
posOrdem:
	addi $sp, $sp, -8 #Dis espacos um para o endereco de retorno e outro para o endereco do node
	sw $ra, 0($sp)
	sw $a1, 4($sp) 	#Gravando endereco de  mem�ria na pilha
	lw $t0, 28($a1) #pegando o endereco do filho a direita
	beq $t0, 0, poFEsq # se o filho mais a direita for nulo d� um branch para verificar o filho mais a esquerda
...
#$a1 -> O endereco do buffer o qual ser� usado para armazenar a cadeia
#$t3 -> rtegistrador contador do tamanho da string
#$t0, $t1, $t2 -> registradore auxiliares para aconvers�o
escreverValor:	 
	addi $sp, $sp, -4
	sw $ra, 0($sp) 
	...
	addi $a1, $a1, 3 #Come�ando no final do buffer da strings
	sb $zero, 0($a1) #Gravando bite nulo
convIntChar:
	divu $t0, $t1 	#Dividindo o valor pelo divisor, pois � a primeiraoperacao
	...
	bne $t0, 0,convIntChar #pr�xima iteracao
endConv:	
